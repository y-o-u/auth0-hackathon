...
```bash
npm run serve
docker run --network lambda-local --name dynamodb-local -d -p 8000:8000 amazon/dynamodb-local -jar DynamoDBLocal.jar -inMemory -sharedDb
sam local start-api -p8080 --docker-network lambda-local --profile default 
